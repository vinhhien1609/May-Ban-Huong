...
*/
int32_t  SSPSendCommand(const SSP_PORT,SSP_COMMAND* cmd);
...
*/
int32_t DownloadFileToTarget(const char * file, const char * port, const unsigned char sspAddress, const long long key);
...
Notes:
    ...
    Only one download operation may be in progress at once.
*/
int32_t DownloadDataToTarget(const unsigned char* data, const uint32_t dlength, const char * cPort, const unsigned char sspAddress,const long long key);
...
*/
int32_t NegotiateSSPEncryption(SSP_PORT port, const char ssp_address, SSP_FULL_KEY * key);
...
Inputs:
    ...
    long long fixedkey: The fixed key to be used for SSP Encryption. The default key is 0x0123456701234567
Return:
...
*/
SSP_RESPONSE_ENUM ssp_setup_encryption(SSP_COMMAND_SETUP * setup,const long long fixedkey);
...
Notes:
*/
SSP_RESPONSE_ENUM ssp_get_serial(SSP_COMMAND_SETUP setup,uint32_t * serial );
