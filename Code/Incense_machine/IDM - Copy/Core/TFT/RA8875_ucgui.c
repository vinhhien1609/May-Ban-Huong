...
void Drive_Line(int x0, int y0, int x1, int y1)
{
   ...
   else
   {
	...
	LCD_CmdWrite(0x63);
	LCD_DataWrite((LCD_COLORINDEX&0xf800)>>11);
	
    LCD_CmdWrite(0x64);
	LCD_DataWrite((LCD_COLORINDEX&0x07e0)>>5);
	
	LCD_CmdWrite(0x65);
	LCD_DataWrite((LCD_COLORINDEX&0x001f));
...
void LCD_L0_DrawHLine  (int x0, int y,  int x1)
{
  ...
  Drive_Line(x0, y, x1, y);	//ra8875 hardware draw line
}

void LCD_L0_DrawVLine  (int x, int y0,  int y1)
{
  //GUI_Line(x,y0,x,y1,LCD_COLORINDEX);
   Drive_Line(x, y0, x, y1); //ra8875 hardware draw line
}
