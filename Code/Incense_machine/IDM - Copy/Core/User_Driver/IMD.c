...
//				NV11_Disable(true);
	...
	if(HAL_GPIO_ReadPin(DOOR_GPIO_Port, DOOR_Pin))	IDM_Status.isDoorOpen = true;
	else	IDM_Status.isDoorOpen = false;
	
		if(HAL_GPIO_ReadPin(Empty_GPIO_Port, Empty_Pin)==0)	IDM_Status.isEmptyIsenseSW = false;
